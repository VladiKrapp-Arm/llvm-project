...
// ARMv6t2_THREAD_POINTER_UNSUPP: hardware TLS register is not supported for the armv6t2 sub-architecture
...
// RUN: not %clang --target=armv6-linux -mtp=cp15 -### -S %s 2>&1 | \
